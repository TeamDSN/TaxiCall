...
local providers = xml:loadFile( "data.xml" )

function parseXMLData()
	local provider = {}

	-- for each "child" in the inbox table...
	for i=1,#providers.child do
	
		-- store the address in the provider table
		provider[i] = providers.child[i]

		-- extract data from table and store in local variables
		-- for easier readability/access:
		local brand = provider[i].child[1].value
		local phone = provider[i].child[2].value
